...
namespace CANStream
{
    public class CS_GlobalSeriesProperties
    {
    	#region Public members
    	
    	public List<GraphSerieProperties> GlobalSeries;
    	
    	#endregion
    	
    	public CS_GlobalSeriesProperties()
    	{
    		GlobalSeries =  new List<GraphSerieProperties>();
    	}
    	
    	#region Public methods
    	
    	public void Write_GlobalSeriesFile(string fPath)
    	{
    		XmlDocument oXDoc = new XmlDocument();
    		
    		XmlElement xSeries = oXDoc.CreateElement("GlobalSeriesProperties");
    		
    		foreach (GraphSerieProperties oSerie in GlobalSeries)
    		{
    			XmlElement xSerie = oSerie.Write_PropertiesXmlNode(oXDoc, "SerieProperties");
    			
    			if (!(xSerie == null))
    			{
    				xSeries.AppendChild(xSerie);
    			}
    		}
    	...
    	public bool Read_GlobalSeriesFile(string fPath)
    	{
    		try
    		{
    			XmlDocument oXDoc = new XmlDocument();
    			
    			XmlNode xSeries = oXDoc.SelectSingleNode("GlobalSeriesProperties");
    			GlobalSeries = new List<GraphSerieProperties>();
    			
    			foreach (XmlNode xSerie in xSeries.ChildNodes)
    			{
    				GraphSerieProperties oSerie = new GraphSerieProperties();
    				
    				if (oSerie.Read_PropertiesXmlNode(xSerie))
    				{
    					GlobalSeries.Add(oSerie);
    				}
    			}
    		}
    		catch
